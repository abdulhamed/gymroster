...
angular.module('vsGoogleAutocomplete').factory('vsGooglePlaceUtility', function() {
	...
	function getSubsubLocality(place) {
		var COMPONENT_TEMPLATE = { sublocality_level_2: 'long_name' },
			subsubLocality = getAddrComponent(place, COMPONENT_TEMPLATE);
	...
	return {
		...
		getSubsubLocality: getSubsubLocality
	...
	return {
		...
		controller: ['$scope', '$attrs', function($scope, $attrs) {
			...
			this.updatePlaceComponents = function(place) {
				...
				$scope.vsSublocality   = !!$attrs.vsSublocality && place  ? vsGooglePlaceUtility.getSubLocality(place)  : undefined;
				$scope.vsSubsublocality   = !!$attrs.vsSubsublocality && place  ? vsGooglePlaceUtility.getSubsubLocality(place)  : undefined;
